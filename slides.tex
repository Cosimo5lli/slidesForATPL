...
\begin{document}
  ...
  \AtBeginSection[]{% Print an outline at the beginning of sections
	...
	\begin{frame}{Preparing for extension}
	% \framesubtitle{Adding some auxiliary functions}
	We then define a new function $\Omega in$ that is usefull to evaluate if the type of a method is in a groups of interfaces and we redefine $mtype$ and $mbody$ also for group of interfaces.
	\end{frame}

	\subsubsection{$\Omega in$ function}

	\begin{frame}{$\Omega in$ function}
\framesubtitle{For groups of interfaces}
\begin{definition}
\vspace{-20pt}
\begin{flalign*}
&\Omega in(m, IObject)=\emptyset &\\[10pt]
% &\dfrac{\mathlarge{
%     CT(I)=\einterface{I}[J] \atop
%     \sig(\aargs{J}) = \aargs{S}
% }}{\sig(I) = \aargs{S} \cup \aargs{N} \hs} &\\[10pt]
...
%     I \in \aargs{I} \quad \sig(I) = \aargs S \quad \sig(\aargs I \setminus \{I\}) = \aargs R
% }}{\sig(\aargs I) = \aargs S \cup \aargs R}
& \Omega in(m,\aargs I)= \dfrac{
\mathlarge{
	mtype(m,\aargs I/\{I_y\}) \vee mtype(m,I_y)
}
}{mtype(m,(\aargs I/\{I_y\}) \bigcup I_y )}&\\[10pt]
\end{flalign*}
\end{definition}
\end{frame}
...
\subsubsection{mtype function}
\begin{frame}
We introduce the changes to the function mtype to make it works also in presence of intersection types.
\frametitle{mtype function}
\end{frame}
\begin{frame}
\frametitle{mtype function}
\framesubtitle{For interfaces with intersection types}
\begin{definition}
\vspace{-20pt}
\begin{flalign*}
% &\dfrac{
...
&CT(I)=\dfrac{
\mathlarge{
Interface\ I\  extends \aargs{I}\{\aargs{N}\} \atop
C\ m(\aargs C\ \aargs x)\in\aargs N
}
}{mtype(m, I)=\aargs C \rightarrow C}& \\[10pt]
&\dfrac{
	\mathlarge{
		Interface\ I\  extends \aargs{I}\{\aargs{N}\} \atop
		m\ not\ defined\ in\ \aargs N
	}
}{mtype(m, I)=\Omega in(m, \aargs{I})}&
% \\[5pt]
% &\dfrac{
%   % \mathlarge
%   {
...
\begin{frame}
\frametitle{mtype function}
\framesubtitle{For classes with intersection types}
\begin{definition}
	\vspace{-20pt}
	\begin{flalign*}
	% &\dfrac{
	...
	&CT(C)=\dfrac{
		\mathlarge{
			Class\ C\  extends\ D\ implements \aargs{I}\{\aargs{T}\aargs{f};\ K\ \aargs{M}\} \atop
			C\ m(\aargs C\ \aargs x)\in\aargs M
		}
	}{mtype(m, C)=\aargs C \rightarrow C}& \\[10pt]
	&\dfrac{
		\mathlarge{
			Class\ C\  extends\ D\ implements \aargs{I}\{\aargs{T}\aargs{f};\ K\ \aargs{M}\} \atop
			m\ is\ not\ defined\ in\ \aargs M
		}
	}{mtype(m, C)=mtype(m, D) \vee mtype(m, C)=\Omega in(m, \aargs{I})}&
	% \\[5pt]
	% &\dfrac{
	%   % \mathlarge
	%   {
